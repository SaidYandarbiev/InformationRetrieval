...
def preprocess_text(text, analyzer):
    ...
    # Use the provided analyzer to process the text
    tokenStream = analyzer.tokenStream("fieldName", StringReader(text))
    tokenStream.reset()  # Initialize the token stream
    
    try:
        while tokenStream.incrementToken():
            tokenized_terms.append(tokenStream.getAttribute(CharTermAttribute.class_).toString())
        tokenStream.end()  # Finalize the token stream
    finally:
        tokenStream.close()  # Ensure the token stream is closed
